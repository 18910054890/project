...
public class RabbitConfiguration {
    ...
    @Autowired
	public RabbitConfiguration(OMRabbitProperties oMRabbitProperties,
    ...
    public TopicExchange publishExchange() {
        return new TopicExchange(oMRabbitProperties.getPublishx(),
                true,
                false,
                of("alternate-exchange", oMRabbitProperties.getPublishdlx()));
    }

    //"发送方DLX"，消息发送失败时传到该DLX
    @Bean
    public TopicExchange publishDlx() {
        return new TopicExchange(oMRabbitProperties.getPublishdlx(), true, false, null);
    }

    //"发送方DLQ"，所有发到"发送DLX"的消息都将路由到该DLQ
    @Bean
    public Queue publishDlq() {
        return new Queue(oMRabbitProperties.getPublishdlq(),
                true,
                false,
                false,
    ...
    public Queue receiveQ() {
        ImmutableMap<String, Object> args = of(
                ...
                oMRabbitProperties.getReceivedlx(),
                "x-overflow",
                "drop-head",
                "x-max-length",
                300000,
                "x-message-ttl",
                24 * 60 * 60 * 1000);
        return new Queue(oMRabbitProperties.getReceiveq(), true, false, false, args);
    }
    ...
    public TopicExchange receiveDlx() {
        return new TopicExchange(oMRabbitProperties.getReceivedlx(), true, false, null);
    }
    ...
    public Queue receiveDlq() {
        return new Queue(oMRabbitProperties.getReceivedlq(), true, false, false, of("x-queue-mode", "lazy"));
    }
    ...
    public TopicExchange receiveRecoverExchange() {
        return new TopicExchange(oMRabbitProperties.getReceiveRecoverx(), true, false, null);
    }
